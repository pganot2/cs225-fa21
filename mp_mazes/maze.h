...
/**
 * Each SquareMaze object represents
 * a randomly-generated square maze and its solution.
 * 
 * (Note that by "square maze" we mean a maze in 
 * which each cell is a square; the maze itself need not be a square.)
 */
class SquareMaze 
{
  public:
     ...
     * until there are no more walls that could be deleted without
     ...
     * it is possible to travel in the given direction from
     * the square at coordinates (x,y).
     * 
     ...
     * It will also be used by the grading program to verify that your 
     * maze is a tree that occupies the whole grid,
     ...
     * You can assume that in grading we will 
     ...
     * for k from 0 to 10. If the bottom wall exists, then blacken the pixels with coordinates
     ...
     * it modifies the PNG from drawMaze to show the solution vector
     * and the exit. 
     * 
  ...
  private:
    /** 1D Vector Maze representation for setting walls *
     *  (pair<bool right, bool bottom>)
    */
    vector<pair<bool, bool>> walls;

    /** DisjointSets set for Cycle Detection **/
    DisjointSets dsets;
